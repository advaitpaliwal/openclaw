...
vi.mock("../../agents/agent-scope.js", () => ({
  listAgentIds: () => ["main", "test-agent", "fancy-agent"],
...
describe("agents mutate handlers", () => {
  ...
  it("agents.create stores avatar/emoji in config identity (does not append IDENTITY.md)", async () => {
  ...
  it("agents.delete trashes files by default", async () => {
    mocks.findAgentEntryIndex.mockReturnValue(0);
    ...
    expect(mocks.runCommandWithTimeout).toHaveBeenCalled();
  });

  it("agents.delete skips trash when deleteFiles=false", async () => {
    mocks.findAgentEntryIndex.mockReturnValue(0);
